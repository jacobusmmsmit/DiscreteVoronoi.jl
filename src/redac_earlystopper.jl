...
"""
    redac_voronoi!(grid, sites::T; p=2, predicate=exact_condition)  where {T<:Vector{SVector{2,Int}}}
...
"""
function redac_voronoi!(grid, sites::T; p=2, predicate=exact_condition) where {T<:Vector{SVector{2,Int}}}
    TL = 1, 1
    BR = size(grid)
