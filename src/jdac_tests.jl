export jdac!, jdacx!, jdac_aux0!, jdac_aux1a!, jdac_aux1b!, jdac_aux1c!, jdac_aux2a!, jdac_aux2b!, jdac_aux2c!, jdac_aux3!, jdac_aux5a!
...
@inbounds function jdac_aux1a!(grid, sites, p, depth, rect, stack)
    ...
    if all(>(0), (N, M)) && any(==(0), @view grid[t:t+N-1, l:l+M-1])
        center = (t + N / 2, l + M / 2)
        max_dist = findmin(site -> distance(center, site[2], p), sites)[1] + norm((N, M), p) + 1
...
@inbounds function jdac_aux1b!(grid, sites, p, depth, rect, stack)
    ...
    if all(>(0), (N, M)) && any(==(0), @view grid[t:t+N-1, l:l+M-1])
        ...
        max_dist = findmin(dists)[1] + norm((N, M), p) + 1
...
@inbounds function jdac_aux1c!(grid, sites, p, depth, rect, stack)
    ...
    if all(>(0), (N, M)) && any(==(0), @view grid[t:t+N-1, l:l+M-1])
        ...
        max_dist = findmin(get_dists(stack))[1] + norm((N, M), p) + 1
...
@inbounds function jdac_aux3!(grid, sites, p, depth, rect, stack)
    (t, l), (N, M) = rect
    if all(>(0), (N, M)) && any(==(0), @view grid[t:t+N-1, l:l+M-1])
        center = (t + N / 2, l + M / 2)
        ...
        jdac!(grid, stack_sites, jdac_aux3!, p, depth - 1, rect, stack)
    end
    grid
end
