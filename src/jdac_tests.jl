...
function jdac_aux1!(grid, sites, p, depth, stack)
    if all(.>(0), size(grid)) && any(==(0), grid)
        ...
        max_dist = findmin(site -> distance(center, site[2], p), sites)[1] + distance((0, 0), size(grid), p) + 1
...
function jdac_aux2!(grid, sites, p, depth, stack)
    if all(.>(0), size(grid)) && any(==(0), grid)
        ...
        max_dist = findmin(dists)[1] + distance((0, 0), size(grid), p) + 1
...
function jdac_aux3!(grid, sites, p, depth, stack)
    if all(.>(0), size(grid)) && any(==(0), grid)
        ...
        max_dist = findmin(get_dists(stack))[1] + distance((0, 0), size(grid), p) + 1
...
function jdac_aux4!(grid, sites, p, depth, stack)
    if all(.>(0), size(grid)) && any(==(0), grid)
        ...
        max_dist = maximum(corner -> distance(corner, min_site, p), corners) + distance((0, 0), center, p) + 1
        stack_sites = filter(site -> distance(center, site[2], p) <= max_dist, sites)
...
function jdac_aux6!(grid, sites, p, depth, stack)
    if all(.>(0), size(grid)) && any(==(0), grid)
        ...
        corners = ((1, 1), (N, 1), (1, M), (N, M))
        max_dist = maximum(corner -> distance(corner, min_site[2], p), corners) + distance((0, 0), center, p) + 1
        fill_sites!(max_dist, sites, stack)
...
function jdac!(grid, sites, aux!, p::Real=2, depth::Int=1, stack=SiteStack{Float64, eltype(sites)}()) 
    N, M = size(grid)
    corners = ((1, 1), (N, 1), (1, M), (N, M))
    nearest_colors = map(corner -> findmin(site -> distance(corner, site[2], p), sites)[2], corners)
